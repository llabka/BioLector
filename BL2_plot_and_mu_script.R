...
csvFiles = "166-001_growth_LLABKA-LLABKA-2022-10-28-07-59-54.csv"
...
#how to read the BL1 csv files, with design table
#pr1 = read_biolector2(file=csvFiles, designfile=desFiles, measureType="AMP",  blVersion=2)
#how to read the BL2 csv files, with design table
...
plot_biolector(x=prsn, spar=0.4, log="y", ylim=c(0.5, 15))
names(pr2)
...
growth_avg_data <- growth_data %>% group_by(interval, Strain, time, Condition) %>%
...
strain_vec <- unique(growth_data_noblanks$Strain)
